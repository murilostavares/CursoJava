package br.com.cursojava.aula002;

import java.util.Scanner;

public class ConceitoNota {
	public static void main(String[] args) {
		System.out.println("Insira 4 notas para que seja calculada a média");

		Scanner teclado1 = new Scanner(System.in);
		System.out.println("Insira a primeira nota");
		String nota1 = teclado1.nextLine();
		int notam1 = Integer.parseInt(nota1);
		System.out.printf("A primeira nota informada foi %s. \n", nota1);

		Scanner teclado2 = new Scanner(System.in);
		System.out.println("Insira a segunda nota");
		String nota2 = teclado2.nextLine();
		int notam2 = Integer.parseInt(nota2);
		System.out.printf("A segunda nota informada foi %s. \n", nota2);

		Scanner teclado3 = new Scanner(System.in);
		System.out.println("Insira a terceira nota");
		String nota3 = teclado3.nextLine();
		int notam3 = Integer.parseInt(nota3);
		System.out.printf("A terceira nota informada foi %s. \n", nota3);

		Scanner teclado4 = new Scanner(System.in);
		System.out.println("Insira a terceira nota");
		String nota4 = teclado3.nextLine();
		int notam4 = Integer.parseInt(nota4);
		System.out.printf("A terceira nota informada foi %s. \n", nota4);

		System.out.println("");

		System.out.println("A média dos valores informados é:");

		double media = (notam1 + notam2 + notam3 + notam4) / 4.0;

		System.out.println(media);

		if (media >= 9) {
			System.out.println("");
			System.out.println("CONCEITO A");
		} else if (media >= 8) {
			System.out.println("");
			System.out.println("CONCEITO B");
		} else if (media >= 6) {
			System.out.println("");
			System.out.println("CONCEITO C");
		} else if (media >= 5) {
			System.out.println("");
			System.out.println("CONCEITO D");
		} else if (media < 5) {
			System.out.println("");
			System.out.println("INSUFICIENTE");
		}

		teclado1.close();
		teclado2.close();
		teclado3.close();
		teclado4.close();
	}

}
